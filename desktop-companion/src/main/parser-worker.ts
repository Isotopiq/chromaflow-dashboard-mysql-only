...
// This is a direct port of the browser mzml.worker.ts — same parsing logic,
// same scans blob format, same peak picker — adapted to read from the
// filesystem and communicate via worker_threads instead of postMessage.
...
export async function parseMzML(text: string): Promise<{ summary: WorkerRunSummary; scansBlob: Uint8Array; ms2Blob: Uint8Array }> {
  ...
  async function main() {
    try {
      ...
      parentPort?.postMessage(
        { ok: true, summary, scansBlob, ms2Blob },
        [scansBlob.buffer as ArrayBuffer, ms2Blob.buffer as ArrayBuffer],
      );
    } catch (err: any) {
